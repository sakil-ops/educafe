  ...
  header("Location:index.php");
    ...
    if($dbh){
        ...
        if(isset($_POST['submit'])){
             ...
             * Sql Query
             */
            $sql = "UPDATE `teachers` SET `name`=:name,`email`=:email,`phone`=:phone,
            `designation`=:designation,`dob`=:dob,
            `gender`=:gender,`religion`=:religion,
            `e_qualification`=:e_qualification,`address`=:address,`f_link`=:f_link,
            `t_link`=:t_link,`g_link`=:g_link,`i_link`=:i_link,`l_link`=:l_link,
            `short_description`=:short_description,
            `long_description`=:long_description,`joining_date`=:joining_date,
            `status`=:status,`image`=:image,`password`=:password,
             WHERE `id`=:id";
